...
import { campaigns, Campaign, getVisualStatus } from '@/data/campaigns';
...
const Index = () => {
  ...
  const [statsMap, setStatsMap] = useState<Record<string, { sold: number; total_numbers: number }>>({});

  useEffect(() => {
    ticketsAPI.getCampaignsStats()
      .then((data) => setStatsMap(data))
      .catch(() => {});
  }, []);

  // Merge real DB stats with mock data
  const enriched = useMemo<Campaign[]>(() =>
    campaigns.map(c => {
      const s = statsMap[c.id];
      return s ? { ...c, soldTickets: s.sold, totalTickets: s.total_numbers } : c;
    }),
    [statsMap]
  );

  // Get featured campaign for hero
  const featuredCampaign = enriched.find(c => c.featured) || enriched[0];

  // Filter and sort campaigns
  const filteredCampaigns = useMemo(() => {
    let result = enriched.filter(c => !c.featured); // Exclude featured from grid
    ...
    return result;
  }, [searchQuery, statusFilter, sortOption, enriched]);
